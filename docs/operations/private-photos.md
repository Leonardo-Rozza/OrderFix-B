...
- `SELECT, INSERT` sobre `public.reparacion_fotos_privadas`, `public.reparacion_foto_atestaciones` y `public.reparacion_foto_eliminaciones`.
- `UPDATE(asset_id,asset_version,identificada_en,resultado,observada_en,confirmada_en)` sobre `public.reparacion_foto_eliminaciones`.
...
No conceder DELETE/TRUNCATE sobre ninguna de esas tres tablas, UPDATE de los vínculos/manifestaciones, ni EXECUTE sobre `foto_privada_conservar_objetos_v30()`, `foto_eliminacion_guard_v35()`, `foto_eliminada_recibo_guard_v35()` o `foto_eliminacion_completa_v35()`. Este último guard se ejecuta por el trigger referencial con privilegios de su propietario; su definición exacta, search_path fijo y propietario de migraciones/tablas se acreditan. No exige conceder lectura de fotos a los consumidores históricos para borrar una reparación sin objetos pendientes. El rechazo general de funciones SECURITY DEFINER ejecutables permanece activo. Los guards V33/V34/V35 se ejecutan por trigger con su propietario; el rol de fotos no recibe EXECUTE sobre ellos ni acceso a las tres tablas de confirmaciones, operaciones y efectos de cierre. El verificador acredita ese delta y rechaza grants adicionales.
...
La aplicación tiene `@EnableScheduling`; con el flag activo, la tarea espera 60 segundos al arrancar y otros 60 después de terminar cada pase. Cada pase selecciona como máximo diez filas vencidas, fallidas, expiradas o pendientes de limpieza, y omite leases todavía vigentes. Un cursor en memoria por instancia recorre el orden `(confirmado_en,id)` y avanza también ante fallos; cuando no quedan candidatos posteriores vuelve al comienzo. Cada pase procesa una sola página; el regreso puede hacer una segunda consulta limitada. Reiniciar la instancia reinicia el cursor sin alterar las filas ni sus controles transaccionales. Los leases de upload/finalización vencen a los dos minutos. También se elimina el archivo asociado al alcanzar `retener_hasta`. El borrado lógico terminal conserva la intención y su vínculo de evidencia legal. Limpia la identidad del asset en la foto, pero V35 la conserva en el objetivo de eliminación cuando pudo acreditarse. No purga el ledger legal.

DELETE y cleanup confirman un objetivo durable antes de cualquier borrado remoto.
Si se descubre un objeto cuya identidad no se había guardado, se acreditan clave,
MIME, bytes y SHA y se confirma esa identidad antes del DELETE. Se reutiliza el mismo
asset en reintentos; el proveedor se invoca fuera de las transacciones SQL.

Un 204 significa finalización local (o replay de una foto ya eliminada). El registro
V35 distingue `IDENTIDAD_ELIMINADA`, con identidad conocida y ausencia confirmada por
el adaptador, de `AUSENCIA_OBSERVADA_SIN_IDENTIDAD`, que sólo observa una clave ausente
sin afirmar que nunca hubo una carga. Esta última no acredita borrado integral y
requiere revisión ante cargas inciertas/tardías. `PENDIENTE` conserva el objetivo
para reintentar después de un error remoto o de confirmación SQL. El resultado y
ELIMINADA se confirman juntos; un replay terminal no cambia fechas ni evidencia.
Las fotos históricas ELIMINADA no reciben constancias retrospectivas. El inventario
de cierre separa confirmadas, ausencias, pendientes y fotos sin recibo; tampoco
certifica backups, CDN, fotos legacy o eliminación de todo el taller.

Un fallo conserva `LIMPIEZA_PENDIENTE` y el objetivo con su identidad para reintentar. El borrado de la reparación se bloquea mientras haya objetos pendientes y devuelve 409 `FOTOS_PRIVADAS_PENDIENTES`, indicando que primero deben eliminarse sus fotos.
