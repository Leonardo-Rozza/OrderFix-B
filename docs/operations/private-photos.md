...
El consumidor de fotos admite únicamente el esquema `public`; su configuración no admite `currentSchema`. La acreditación histórica de V29 en otros esquemas se conserva por separado y no acredita fotos V30 allí. Aplicar las migraciones hasta V34 con la identidad de migraciones; no editar V27–V33. V33 coordina las operaciones con el estado de cierre del taller y V34 agrega confirmaciones y efectos internos; la acreditación de fotos conserva su catálogo y permisos. Debe existir una publicación legal vigente y coherente para `USO_CONTINUADO` y `ATESTACION_FOTOS`, con las audiencias de ADMIN y USER. La declaración FOTOS requerida se vuelve a confirmar por cada intención, sin duplicar los actos legales; cada nueva operación conserva su propio resultado idempotente y vínculo contextual sobre la persistencia legal existente.
