...
/** Verifies the offline editorial role's exact effective PostgreSQL capabilities. */
final class LegalEditorialPrivilegeVerifier implements LegalDatabasePreflight {
    ...
    LegalEditorialPrivilegeVerifier(
            JdbcTemplate jdbc,
            String expectedRole,
    ...
    private void verifyRelationsAndColumns(long roleOid) {
        ...
        expectedRelations.add(LegalV27EditorialInventory.FLYWAY_HISTORY_TABLE);
        ...
        for (RelationState relation : relations) {
            ...
            boolean editorialTable = targetSchema
                    && LegalV27EditorialInventory.EDITORIAL_TABLES.contains(relation.name());
            boolean history = targetSchema
                    && LegalV27EditorialInventory.FLYWAY_HISTORY_TABLE.equals(relation.name());
            if (editorialTable || history) {
                foundRelations.add(relation.name());
            }
            boolean allowedRead = editorialTable || history;
            boolean allowedInsert = targetSchema
                    && LegalV27EditorialInventory.INSERT_TABLES.contains(relation.name());
            boolean allowedDelete = targetSchema
                    && LegalV27EditorialInventory.DELETE_TABLES.contains(relation.name());
            if (relation.owner()
        ...
        for (ColumnState column : columns) {
            ...
            boolean editorialTable = targetSchema
                    && LegalV27EditorialInventory.EDITORIAL_TABLES.contains(column.table());
            boolean history = targetSchema
                    && LegalV27EditorialInventory.FLYWAY_HISTORY_TABLE.equals(column.table());
            boolean allowedInsert = targetSchema
                    && LegalV27EditorialInventory.INSERT_TABLES.contains(column.table());
            boolean allowedUpdate = targetSchema
                    && LegalV27EditorialInventory.UPDATE_COLUMNS
                            .getOrDefault(column.table(), Set.of())
                            .contains(column.name());
            if (column.canSelect() != (editorialTable || history)
                    || column.selectGrant()
    ...
    private void verifySequences(long roleOid) {
        ...
        for (SequenceState sequence : sequences) {
            boolean allowed = expectedSchema.equals(sequence.schema())
                    && LegalV27EditorialInventory.WRITABLE_SEQUENCES
        ...
        if (!found.equals(LegalV27EditorialInventory.WRITABLE_SEQUENCES)) {
            incompatible();
        }
    }
    ...
    private void verifyFunctions(long roleOid) {
        List<FunctionPrivilege> functions = jdbc.query("""
                ...
                resultSet.getBoolean("public_execute")),
                roleOid, roleOid, roleOid);
        Set<String> foundEditorial = new HashSet<>();
        for (FunctionPrivilege function : functions) {
            boolean editorial = expectedSchema.equals(function.schema())
                    && LegalV27EditorialInventory.EDITORIAL_FUNCTIONS
                            .containsKey(function.signature());
            if (editorial) {
                foundEditorial.add(function.signature());
            }
            boolean allowed = editorial
                    && LegalV27EditorialInventory.PRIVILEGED_FUNCTIONS
                            .contains(function.signature());
            if (function.owner()
                    || function.canExecute() != allowed
                    || function.executeGrant()
                    || (editorial && function.publicExecute())) {
                incompatible();
            }
        }
        if (!foundEditorial.equals(
