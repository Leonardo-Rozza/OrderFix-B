...
/**
 * Exact effective capabilities of the isolated, keyless expired-metadata maintenance consumer.
 *
 * <p>The schema preflight is a separate mandatory boundary. This role cannot create acceptance
 * requests, read encrypted metadata or account data, or delete contractual evidence. Invoker
 * constraints require the canonical acceptance graph to remain readable. PostgreSQL checks this
 * independently of whether the maintenance batch happens to contain any candidates.</p>
 */
final class LegalAcceptanceMaintenancePrivilegeVerifier implements LegalDatabasePreflight {

    static final String ISSUE_LOCATION = "database/maintenance-privileges";
    ...
    static final Set<String> READ_TABLES = Set.of(
            ...
            // The frozen completeness helper selects xmin: PostgreSQL requires relation SELECT.
            // These are stored HMACs and version identifiers, never encryption/HMAC key material.
            "legal_idempotencia_sin_actos");
    static final Map<String, Set<String>> SELECT_COLUMNS = Map.of(
            "talleres", Set.of("id"),
            "legal_aceptacion_lotes", Set.of("id", "taller_id", "audiencia"),
            "legal_aceptacion_metadatos_cifrados", Set.of("lote_id", "tipo", "tombstone_en"),
            "legal_idempotencia_resultados", Set.of("id", "taller_id", "user_id", "operacion",
                    "route_template", "scope_hmac", "idempotency_key_hmac", "expires_at"),
            "legal_idempotencia_sin_actos_referencias",
                    Set.of("resultado_id", "aceptacion_id", "taller_id", "user_id"));
    ...
    static final Map<String, Set<String>> UPDATE_COLUMNS = Map.of(
            "legal_aceptacion_metadatos", Set.of("purgado_en"),
            "legal_aceptacion_metadatos_cifrados",
                    Set.of("ciphertext", "tag", "longitud_original", "tombstone_en"),
            // FOR UPDATE / SHARE only; the frozen guards still reject any ledger UPDATE.
            "legal_idempotencia_resultados", Set.of("id"),
    ...
    static final Set<String> PRIVILEGED_FUNCTIONS = Set.of(
            ...
            "legal_metadata_cifrada_update_guard()", "legal_metadata_header_update_guard()",
    ...
    private final Set<String> readTables = READ_TABLES;
    private final Set<String> insertTables = INSERT_TABLES;
    private final Map<String, Set<String>> selectColumns = SELECT_COLUMNS;
    private final Map<String, Set<String>> updateColumns = UPDATE_COLUMNS;
    private final Set<String> schemaFunctions = SCHEMA_FUNCTIONS;
    private final Set<String> privilegedFunctions = PRIVILEGED_FUNCTIONS;
    private final JdbcTemplate jdbc;
    private final String expectedRole;
    private final String expectedSchema;

    LegalAcceptanceMaintenancePrivilegeVerifier(
            JdbcTemplate jdbc, String expectedRole, String expectedSchema) {
    ...
    public void verify() {
        ...
        verifyRelationsAndColumns(role.oid(), selectColumns);
    ...
    private void verifyRelationsAndColumns(long roleOid, Map<String,Set<String>> selectColumns) {
        ...
        for (RelationState relation : relations) {
            ...
            boolean allowedDelete = targetSchema && DELETE_TABLES.contains(relation.name());
            if (relation.owner()
