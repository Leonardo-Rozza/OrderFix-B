 ...
 * observation is returned, including when commit/cleanup completed too late.
 */
final class LegalPublicDocumentDataSource extends AbstractDataSource implements AutoCloseable {
    ...
    private final ThreadLocal<LegalPublicDocumentDeadline> current = new ThreadLocal<>();
    ...
    private final ScheduledThreadPoolExecutor watchdog = new ScheduledThreadPoolExecutor(2, task -> {
        Thread thread = new Thread(task, "legal-document-deadline");
        thread.setDaemon(true);
        return thread;
    });

    LegalPublicDocumentDataSource(DataSource pool, Duration operationBudget) {
        this.pool = Objects.requireNonNull(pool, "pool");
        this.operationBudget = Objects.requireNonNull(operationBudget, "operationBudget");
        new LegalPublicDocumentDeadline(operationBudget);
        watchdog.setRemoveOnCancelPolicy(true);
    }

    <T> T withinDeadline(Function<LegalPublicDocumentDeadline, T> operation) {
        LegalPublicDocumentDeadline previous = current.get();
        LegalPublicDocumentDeadline deadline = previous == null
                ? new LegalPublicDocumentDeadline(operationBudget) : previous;
    ...
    public Connection getConnection() throws SQLException {
        requireOpen();
        LegalPublicDocumentDeadline deadline = current.get();
        if (deadline == null) {
            throw new SQLException("La conexión documental requiere un presupuesto activo");
        }
    ...
    public Connection getConnection(String username, String password) throws SQLException {
        throw new SQLFeatureNotSupportedException("La credencial documental es fija");
    }
    ...
    private final class Lease {
        ...
        private Lease(Connection connection, LegalPublicDocumentDeadline deadline) throws SQLException {
                    ...
                    new Class<?>[]{Connection.class}, (proxy, method, args) -> {
                        ...
                        synchronized (this) {
                            if (released) {
                                ...
                                throw new SQLException("La conexión documental está cerrada");
                            }
                        }
                        if (name.equals("unwrap")) {
                            throw new SQLException("La conexión documental no expone su delegado");
                        }
        ...
        private Object statementProxy(Statement statement) {
            ...
            return Proxy.newProxyInstance(type.getClassLoader(), new Class<?>[]{type}, (proxy, method, args) -> {
                ...
                String name = method.getName();
                if (name.equals("unwrap")) {
                    throw new SQLException("La sentencia documental no expone su delegado");
                }
                if (name.equals("isWrapperFor")) {
                    return false;
                }
        ...
        private ResultSet resultSetProxy(ResultSet rows, Statement statement) {
            return (ResultSet) Proxy.newProxyInstance(ResultSet.class.getClassLoader(),
                    new Class<?>[]{ResultSet.class}, (proxy, method, args) -> {
                        ...
                        String name = method.getName();
                        if (name.equals("unwrap")) {
                            throw new SQLException("El cursor documental no expone su delegado");
                        }
                        if (name.equals("isWrapperFor")) {
                            return false;
                        }
    ...
    private static Object objectMethod(Object proxy, Method method, Object[] args) {
        return switch (method.getName()) {
            ...
            case "toString" -> "LegalPublicDocumentJdbc[restricted]";
