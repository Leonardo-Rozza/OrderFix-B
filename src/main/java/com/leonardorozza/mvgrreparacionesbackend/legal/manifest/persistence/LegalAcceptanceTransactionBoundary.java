...
final class LegalAcceptanceTransactionBoundary {
    ...
    private final LegalAcceptancePrivilegeVerifier privileges;

    LegalAcceptanceTransactionBoundary(JdbcTemplate jdbc, LegalPrivateRequirementsDataSource dataSource,
            TransactionTemplate transaction, LegalDatabaseBudgets budgets,
            LegalV29AcceptanceSchemaVerifier schema, LegalAcceptancePrivilegeVerifier privileges) {
    ...
    void requireExactBoundary() {
        ...
        if (candidate == null || candidate.getClass() != DataSourceTransactionManager.class
                ...
                || budgets.transactionTimeoutSeconds() != 15 || budgets.statementTimeoutSeconds() != 5
            ...
            throw new IllegalArgumentException("La aceptación requiere una frontera JDBC aislada y acreditable");
        }
    }
    ...
    private void requireEffectiveTransaction() {
        if (!TransactionSynchronizationManager.isActualTransactionActive()
            ...
            throw new IllegalStateException("La aceptación requiere su transacción mutable activa");
        }
        try {
            ...
            if (holder.isRollbackOnly() || connection.getAutoCommit() || connection.isReadOnly()
                ...
                throw new IllegalStateException("La aceptación requiere READ_COMMITTED efectivo");
            }
            ...
            if (!"read committed".equals(mode.get("isolation")) || !"off".equals(mode.get("read_only"))) {
                throw new IllegalStateException("La aceptación requiere modo PostgreSQL mutable READ_COMMITTED");
            }
        } catch (SQLException failure) {
            throw new IllegalStateException("No se pudo acreditar la transacción de aceptación", failure);
        }
    }
