...
final class LegalPublicRequirementsDataSource extends AbstractDataSource implements AutoCloseable {
    ...
    private final ThreadLocal<LegalPublicRequirementsDeadline> current = new ThreadLocal<>();
    ...
    private final ScheduledThreadPoolExecutor watchdog = new ScheduledThreadPoolExecutor(2, task -> {
        Thread thread = new Thread(task, "legal-public-requirements-deadline");
        thread.setDaemon(true);
        return thread;
    });

    LegalPublicRequirementsDataSource(DataSource pool, Duration operationBudget) {
        this(pool, operationBudget, System::nanoTime);
    }

    /** Package-private clock seam for deterministic phase/commit boundary tests. */
    LegalPublicRequirementsDataSource(DataSource pool, Duration operationBudget, LongSupplier clock) {
        this.pool = Objects.requireNonNull(pool, "pool");
        this.operationBudget = Objects.requireNonNull(operationBudget, "operationBudget");
        this.clock = Objects.requireNonNull(clock, "clock");
        new LegalPublicRequirementsDeadline(operationBudget, clock);
        watchdog.setRemoveOnCancelPolicy(true);
    }

    <T> T withinDeadline(Function<LegalPublicRequirementsDeadline, T> operation) {
    ...
    public Connection getConnection() throws SQLException {
        requireOpen();
        LegalPublicRequirementsDeadline deadline = current.get();
        if (deadline == null) {
    ...
    private final class Lease {
        ...
        private final LegalPublicRequirementsDeadline deadline;
        ...
        private Lease(Connection connection, LegalPublicRequirementsDeadline deadline) throws SQLException {
    ...
    private static Object objectMethod(Object proxy, Method method, Object[] args) {
        return switch (method.getName()) {
            ...
            case "toString" -> "LegalPublicRequirementsJdbc[restricted]";
