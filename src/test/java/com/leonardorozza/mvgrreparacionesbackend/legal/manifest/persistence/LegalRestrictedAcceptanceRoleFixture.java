...
/**
 * Provisions the exact acceptance consumer in an ephemeral test database.
 *
 ...
 */
final class LegalRestrictedAcceptanceRoleFixture {

    private static final String SAFE_DATABASE_PREFIX = "ordenfix_legal_acceptance_";
    private static final String DEFAULT_SCHEMA = "public";

    private final JdbcTemplate owner;
    private final Credentials credentials;

    private LegalRestrictedAcceptanceRoleFixture(
            JdbcTemplate owner,
            String jdbcUrl,
            String role,
            String password,
    ...
    static Credentials provision(JdbcTemplate owner, String role, String password) {
        try (var connection = Objects.requireNonNull(owner.getDataSource()).getConnection()) {
            return new LegalRestrictedAcceptanceRoleFixture(owner, connection.getMetaData().getURL(), role,
    ...
    private Credentials provisionAndVerify() {
        ...
        revokeExecuteFromPublicAndRole(
                "pg_catalog",
                LegalAcceptancePrivilegeVerifier.LARGE_OBJECT_CREATION_FUNCTIONS);
        revokeExecuteFromPublicAndRole(
                "pg_catalog",
                LegalAcceptancePrivilegeVerifier.SESSION_ADVISORY_LOCK_FUNCTIONS);
        revokeExecuteFromPublicAndRole(
                DEFAULT_SCHEMA,
                LegalAcceptancePrivilegeVerifier.PRIVILEGED_FUNCTIONS);

        owner.execute("GRANT SELECT ON TABLE "
                + qualifiedNames(LegalAcceptancePrivilegeVerifier.READ_TABLES)
                + " TO " + quoteIdentifier(credentials.username()));
        LegalAcceptancePrivilegeVerifier.SELECT_COLUMNS.forEach((table, columns) ->
                owner.execute("GRANT SELECT ("
                        + String.join(", ", columns.stream().sorted()
                                .map(LegalRestrictedAcceptanceRoleFixture::quoteIdentifier).toList())
                        + ") ON TABLE public." + quoteIdentifier(table)
                        + " TO " + quoteIdentifier(credentials.username())));
        owner.execute("GRANT INSERT ON TABLE "
                + qualifiedNames(LegalAcceptancePrivilegeVerifier.INSERT_TABLES)
                + " TO " + quoteIdentifier(credentials.username()));
        LegalAcceptancePrivilegeVerifier.UPDATE_COLUMNS.forEach((table, columns) ->
                owner.execute("GRANT UPDATE ("
                        + String.join(", ", columns.stream()
                                .sorted()
                                .map(LegalRestrictedAcceptanceRoleFixture::quoteIdentifier)
                                .toList())
                        + ") ON TABLE public." + quoteIdentifier(table)
                        + " TO " + quoteIdentifier(credentials.username())));
    ...
    private void verify() {
        ...
        new LegalAcceptancePrivilegeVerifier(
    ...
    static String requireSafeEphemeralDatabase(JdbcTemplate owner) {
        ...
        if (database == null || !database.startsWith(SAFE_DATABASE_PREFIX)) {
            throw new IllegalStateException(
                    "El fixture de aceptaciones sólo puede mutar una base efímera dedicada");
        }
        return database;
    }
    ...
    private void requireAbsentRole() {
        ...
        if (Boolean.TRUE.equals(exists)) {
            throw new IllegalStateException("El rol de aceptaciones efímero ya existe");
        }
    }
    ...
    static void seedCatalog(JdbcTemplate owner, java.nio.file.Path directory, Class<?> source) throws Exception {
        ...
        var release = LegalManifestPersistenceITSupport.copyRelease(directory, source, "acceptance",
                (path, manifest) -> {
