...
class LegalAcceptanceDatabaseConfigurationTest {
    private static final String PREFIX = LegalAcceptanceDatabaseConfiguration.PROPERTY_PREFIX;
    private static final String FLAG = LegalAcceptanceDatabaseConfiguration.ENABLED_PROPERTY;
    ...
    void malformedAcceptanceFlagFails(String flag) {
        try (var context = unregisteredContext(Map.of(FLAG, flag))) {
            assertThatThrownBy(() -> {
                context.register(LegalAcceptanceDatabaseConfiguration.class);
                context.refresh();
            }).isExactlyInstanceOf(IllegalArgumentException.class)
                    .hasMessage("El flag de aceptación legal requiere true o false exactos");
            assertThat(context.isActive()).isFalse();
            assertThat(context.getBeanFactory().containsSingleton("legalAcceptancePool")).isFalse();
        }
    }

    @ParameterizedTest @NullSource @ValueSource(strings = {"false", "TRUE", " true", "1"})
    void enabledRequiresExplicitExactReadFlag(String read) {
        Map<String, Object> values = properties();
        if (read == null) values.remove(LegalPrivateRequirementsDatabaseConfiguration.ENABLED_PROPERTY);
        else values.put(LegalPrivateRequirementsDatabaseConfiguration.ENABLED_PROPERTY, read);
        try (var context = unregisteredContext(values)) {
            assertThatThrownBy(() -> {
                context.register(LegalAcceptanceDatabaseConfiguration.class);
                context.refresh();
            }).isExactlyInstanceOf(IllegalArgumentException.class)
                    .hasMessage("La aceptación legal requiere account-read=true");
            assertThat(context.isActive()).isFalse();
            assertThat(context.getBeanFactory().containsSingleton("legalAcceptancePool")).isFalse();
        }
    }
    ...
    @Test
    void isolatedExactGraphIsLazyClosesAndContainsOnlyAcceptanceInfrastructure() {
        assertThat(AnnotatedElementUtils.hasAnnotation(LegalAcceptanceDatabaseConfiguration.class, Component.class)).isFalse();
        assertThat(LegalAcceptanceDatabaseConfiguration.class.isAnnotationPresent(Configuration.class)).isFalse();
        ...
        try (var context = context(properties())) {
            ...
            assertThat(tx.getTimeout()).isEqualTo(15);
            var boundary = context.getBean(LegalAcceptanceTransactionBoundary.class);
            boundary.requireExactBoundary();
            assertThat(boundary.usesJdbc(jdbc)).isTrue();
            assertThat(context.getBean(LegalAcceptanceEvidenceReader.class)).isNotNull();
            assertThat(context.getBean(LegalAcceptanceSelection.class)).isNotNull();
            ...
            assertThat(context.getBean(LegalIdempotencyKeyring.class).resultTtl()).isEqualTo(java.time.Duration.ofHours(25));
    ...
    @Test
    void cannotCombineAcceptanceAndPrivateDatabaseContexts() {
    ...
    static Map<String, Object> properties() {
        ...
        values.put(PREFIX + "jdbc-url", "jdbc:postgresql://127.0.0.1:1/acceptance");
        values.put(PREFIX + "username", "acceptance-fixture");
        values.put(PREFIX + "password", "acceptance-fixture-password");
    ...
    private static AnnotationConfigApplicationContext unregisteredContext(Map<String, Object> values) {
        ...
        context.getEnvironment().getPropertySources().addFirst(new MapPropertySource("acceptance-test", values));
        return context;
    }
