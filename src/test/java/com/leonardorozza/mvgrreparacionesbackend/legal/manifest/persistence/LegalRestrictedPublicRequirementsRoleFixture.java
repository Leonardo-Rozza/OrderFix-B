...
/**
 * Provisions the exact public-requirements consumer in an ephemeral test database.
 *
 ...
 */
public final class LegalRestrictedPublicRequirementsRoleFixture {
    ...
    public LegalRestrictedPublicRequirementsRoleFixture(
            JdbcTemplate owner,
            String jdbcUrl,
            String role,
            String password,
    ...
    public Credentials provisionAndVerify() {
        ...
        revokeExecuteFromPublicAndRole(
                "pg_catalog",
                LegalPublicRequirementsPrivilegeVerifier.LARGE_OBJECT_CREATION_FUNCTIONS);
        revokeExecuteFromPublicAndRole(
                "pg_catalog",
                LegalPublicRequirementsPrivilegeVerifier.SESSION_ADVISORY_LOCK_FUNCTIONS);
        revokeExecuteFromPublicAndRole(
                DEFAULT_SCHEMA,
                LegalPublicRequirementsPrivilegeVerifier.PRIVILEGED_FUNCTIONS);

        owner.execute("GRANT SELECT ON TABLE "
                + qualifiedNames(LegalPublicRequirementsPrivilegeVerifier.READ_TABLES)
                + " TO " + quoteIdentifier(credentials.username()));
        owner.execute("GRANT INSERT ON TABLE "
                + qualifiedNames(LegalPublicRequirementsPrivilegeVerifier.INSERT_TABLES)
                + " TO " + quoteIdentifier(credentials.username()));
        LegalPublicRequirementsPrivilegeVerifier.UPDATE_COLUMNS.forEach((table, columns) ->
                owner.execute("GRANT UPDATE ("
                        + String.join(", ", columns.stream()
                                .sorted()
                                .map(LegalRestrictedPublicRequirementsRoleFixture::quoteIdentifier)
                                .toList())
    ...
    public void verify() {
        ...
        new LegalV28AggregateSchemaVerifier(restricted, DEFAULT_SCHEMA).verify();
        new LegalPublicRequirementsPrivilegeVerifier(
    ...
    private String requireSafeEphemeralDatabase() {
        ...
        if (database == null || !database.startsWith(SAFE_DATABASE_PREFIX)) {
            throw new IllegalStateException(
                    "El fixture de requisitos públicos sólo puede mutar una base efímera dedicada");
        }
        return database;
    }
    ...
    private void requireAbsentRole() {
        ...
        if (Boolean.TRUE.equals(exists)) {
            throw new IllegalStateException("El rol de requisitos públicos efímero ya existe");
        }
    }
