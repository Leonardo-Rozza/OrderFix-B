...
/**
 * Real HTTP policies over a caller-owned, instrumented restricted reader.
 * The caller closes its reader graph; this helper owns only its separate MVC context.
 * The full production bridge is independently exercised by LegalPublicDocumentHttpIT.
 */
final class LegalPublicDocumentHttpITSupport {

    private LegalPublicDocumentHttpITSupport() { }

    static HttpHarness openHttp(LegalPublicDocumentReadService service) {
        Objects.requireNonNull(service, "service");
        AnnotationConfigWebApplicationContext context = new AnnotationConfigWebApplicationContext();
        try {
            context.setServletContext(new MockServletContext());
            context.getEnvironment().getPropertySources().addFirst(new MapPropertySource("legal-http-it",
                    Map.of(LegalPublicDocumentRequestMatcher.ENABLED_PROPERTY, "true")));
            context.addBeanFactoryPostProcessor(factory ->
                    factory.registerSingleton("legalPublicDocumentReadService", service));
