...
/**
 * Provisions the exact keyless maintenance consumer in an ephemeral test database.
 *
 ...
 */
final class LegalRestrictedMaintenanceRoleFixture {

    private static final String SAFE_DATABASE_PREFIX = "ordenfix_legal_maintenance_";
    private static final String DEFAULT_SCHEMA = "public";

    private final JdbcTemplate owner;
    private final Credentials credentials;

    private LegalRestrictedMaintenanceRoleFixture(
            JdbcTemplate owner,
            String jdbcUrl,
            String role,
            String password,
    ...
    static Credentials provision(JdbcTemplate owner, String role, String password) {
        try (var connection = Objects.requireNonNull(owner.getDataSource()).getConnection()) {
            return new LegalRestrictedMaintenanceRoleFixture(owner, connection.getMetaData().getURL(), role,
    ...
    private Credentials provisionAndVerify() {
        ...
        revokeExecuteFromPublicAndRole(
                "pg_catalog",
                LegalAcceptanceMaintenancePrivilegeVerifier.LARGE_OBJECT_CREATION_FUNCTIONS);
        revokeExecuteFromPublicAndRole(
                "pg_catalog",
                LegalAcceptanceMaintenancePrivilegeVerifier.SESSION_ADVISORY_LOCK_FUNCTIONS);
        revokeExecuteFromPublicAndRole(
                DEFAULT_SCHEMA,
                LegalAcceptanceMaintenancePrivilegeVerifier.PRIVILEGED_FUNCTIONS);

        owner.execute("GRANT SELECT ON TABLE "
                + qualifiedNames(LegalAcceptanceMaintenancePrivilegeVerifier.READ_TABLES)
                + " TO " + quoteIdentifier(credentials.username()));
        LegalAcceptanceMaintenancePrivilegeVerifier.SELECT_COLUMNS.forEach((table, columns) ->
                owner.execute("GRANT SELECT ("
                        + String.join(", ", columns.stream().sorted()
                                .map(LegalRestrictedMaintenanceRoleFixture::quoteIdentifier).toList())
                        + ") ON TABLE public." + quoteIdentifier(table)
                        + " TO " + quoteIdentifier(credentials.username())));
        owner.execute("GRANT DELETE ON TABLE "
                + qualifiedNames(LegalAcceptanceMaintenancePrivilegeVerifier.DELETE_TABLES)
                + " TO " + quoteIdentifier(credentials.username()));
        LegalAcceptanceMaintenancePrivilegeVerifier.UPDATE_COLUMNS.forEach((table, columns) ->
                owner.execute("GRANT UPDATE ("
                        + String.join(", ", columns.stream()
                                .sorted()
                                .map(LegalRestrictedMaintenanceRoleFixture::quoteIdentifier)
                                .toList())
                        + ") ON TABLE public." + quoteIdentifier(table)
                        + " TO " + quoteIdentifier(credentials.username())));
    ...
    private void verify() {
        ...
        new LegalAcceptanceMaintenancePrivilegeVerifier(
    ...
    static String requireSafeEphemeralDatabase(JdbcTemplate owner) {
        ...
        if (database == null || !database.startsWith(SAFE_DATABASE_PREFIX)) {
            throw new IllegalStateException(
                    "El fixture de mantenimiento sólo puede mutar una base efímera dedicada");
        }
        return database;
    }
    ...
    private void requireAbsentRole() {
        ...
        if (Boolean.TRUE.equals(exists)) {
            throw new IllegalStateException("El rol de mantenimiento efímero ya existe");
        }
    }
