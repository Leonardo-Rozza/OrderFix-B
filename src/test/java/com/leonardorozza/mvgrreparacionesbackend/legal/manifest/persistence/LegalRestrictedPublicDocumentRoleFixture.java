...
/**
 * Provisions the exact public-document reader in a dedicated ephemeral test database.
 *
 * <p>PUBLIC function ACLs are changed only in this isolated fixture. This is not a shared-database
 * provisioning script: nominal consumers must be inventoried before applying those revocations
 * anywhere that existing roles or application sessions may depend on legacy PUBLIC grants.</p>
 */
public final class LegalRestrictedPublicDocumentRoleFixture {
    ...
    public LegalRestrictedPublicDocumentRoleFixture(
            JdbcTemplate owner,
            String jdbcUrl,
            String role,
            String password,
    ...
    public Credentials provisionAndVerify() {
        ...
        revokeExecuteFromPublicAndRole(
                "pg_catalog",
                LegalPublicDocumentPrivilegeVerifier.LARGE_OBJECT_CREATION_FUNCTIONS);
        revokeExecuteFromPublicAndRole(
                "pg_catalog",
                LegalPublicDocumentPrivilegeVerifier.SESSION_ADVISORY_LOCK_FUNCTIONS);
        owner.execute("GRANT SELECT ON TABLE "
                + qualifiedNames(LegalPublicDocumentPrivilegeVerifier.READ_TABLES)
                + " TO " + quoteIdentifier(credentials.username()));
    ...
    public void verify() {
        ...
        new LegalPublicDocumentPrivilegeVerifier(
    ...
    private String requireSafeEphemeralDatabase() {
        ...
        if (database == null || !database.startsWith(SAFE_DATABASE_PREFIX)) {
            throw new IllegalStateException(
                    "El fixture de lectura pública sólo puede mutar una base efímera dedicada");
        }
        return database;
    }
    ...
    private void requireAbsentRole() {
        ...
        if (Boolean.TRUE.equals(exists)) {
            throw new IllegalStateException("El rol lector público efímero ya existe");
        }
    }
