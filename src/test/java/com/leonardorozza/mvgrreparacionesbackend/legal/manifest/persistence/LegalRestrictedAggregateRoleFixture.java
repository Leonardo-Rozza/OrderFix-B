...
/** Provisions the exact temporary V28 materializer account in a dedicated test database. */
public final class LegalRestrictedAggregateRoleFixture {

    private static final String SAFE_DATABASE_PREFIX = "ordenfix_legal_aggregate_";
    private static final String DEFAULT_SCHEMA = "public";

    private final JdbcTemplate owner;
    private final Credentials credentials;

    public LegalRestrictedAggregateRoleFixture(
            JdbcTemplate owner,
            String jdbcUrl,
            String role,
            String password,
    ...
    public Credentials provisionAndVerify() {
        ...
        revokeExecuteFromPublicAndRole(
                "pg_catalog",
                LegalV28AggregatePrivilegeVerifier.LARGE_OBJECT_CREATION_FUNCTIONS);
        revokeExecuteFromPublicAndRole(
                "pg_catalog",
                LegalV28AggregatePrivilegeVerifier.SESSION_ADVISORY_LOCK_FUNCTIONS);
        revokeExecuteFromPublicAndRole(
                DEFAULT_SCHEMA,
                LegalV28AggregatePrivilegeVerifier.PRIVILEGED_FUNCTIONS);

        owner.execute("GRANT SELECT ON TABLE public.flyway_schema_history, "
                + qualifiedNames(LegalV28AggregatePrivilegeVerifier.READ_TABLES)
                + " TO " + quoteIdentifier(credentials.username()));
        owner.execute("GRANT INSERT ON TABLE "
                + qualifiedNames(LegalV28AggregatePrivilegeVerifier.INSERT_TABLES)
                + " TO " + quoteIdentifier(credentials.username()));
        LegalV28AggregateInventory.UPDATE_COLUMNS.forEach((table, columns) ->
                owner.execute("GRANT UPDATE ("
                        + String.join(", ", columns.stream()
                                .sorted()
                                .map(LegalRestrictedAggregateRoleFixture::quoteIdentifier)
                                .toList())
    ...
    private String requireSafeEphemeralDatabase() {
        ...
        if (database == null || !database.startsWith(SAFE_DATABASE_PREFIX)) {
            throw new IllegalStateException(
                    "El fixture agregado sólo puede mutar una base efímera dedicada");
        }
        return database;
    }
    ...
    private void requireAbsentRole() {
        ...
        if (Boolean.TRUE.equals(exists)) {
            throw new IllegalStateException("El rol agregado efímero ya existe");
        }
    }
